...
int main(int argc, char* argv[]) {
    ...
    double DeltaVX = argc > 2 ? atof(argv[2]) : 0.0001;
    ...
    // We now do the same as above, but vary the testparticle's velocity
    printf("\nShifting testparticle's x velocity by %f.\n", DeltaVX);
    r = create_sim();
    r->particles[2].vx += DeltaVX;
    reb_integrate(r,100.);
    printf("Position of testparticle at t=100 in shifted simulation:       %.8f %.8f\n",r->particles[2].x,r->particles[2].y);
    reb_free_simulation(r);
    ...
    reb_integrate(r,100.);
    printf("Position of testparticle at t=100 using 1st order var. eqs.:   %.8f %.8f\n",r->particles[2].x+DeltaVX*r->particles[var_i].x,r->particles[2].y+DeltaVX*r->particles[var_i].y);
    reb_free_simulation(r);
    ...
    r->particles[var_i].vx = 1.;
    reb_integrate(r,100.);
    printf("Position of testparticle at t=100 using 2nd order var. eqs.:   %.8f %.8f\n",r->particles[2].x+DeltaVX*r->particles[var_i].x+DeltaVX*DeltaVX/2.*r->particles[var_ii].x,r->particles[2].y+DeltaVX*r->particles[var_i].y+DeltaVX*DeltaVX/2.*r->particles[var_ii].y);
    reb_free_simulation(r);
}
