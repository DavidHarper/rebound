...
void heartbeat(struct reb_simulation* const r) {
  ...
  for (int i = 0; i < 5; i++) {
    double l = getLongitude(r, i);
    double d = getDistance(r, i);
    printf("  %9.4f %10.6f", l, d);
  }

  struct reb_orbit o = reb_tools_particle_to_orbit(r->G, r->particles[5], r->particles[0]);

  printf("  %8.4f %8.4f", o.a, o.e);
